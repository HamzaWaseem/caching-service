...
@router.post("/payload")
async def create_payload(list_1: list[str] = None, list_2: list[str] = None, request: Request = None, session: Session = Depends(get_session)):
    ...
    if list_1 is None or list_2 is None:
        raise HTTPException(status_code=400, detail="Both 'list_1' and 'list_2' must be provided.")
    ...
    if existing_payload:
        return {"message": "Payload already exists.", "payload_id": existing_payload.payload_id}
